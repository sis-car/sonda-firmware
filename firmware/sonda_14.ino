...
const char* OTA_FIRMWARE_URL = "https://raw.githubusercontent.com/sis-car/sonda-firmware/refs/heads/main/firmware/sonda_14.bin";
...
void checkAndApplyOTA() {
  ...
  // --- VALIDAR MAGIC BYTE antes de lanzar OTA ---
  // El primer byte de un .bin valido para ESP32 siempre es 0xE9.
  // Si GitHub devuelve HTML (404/redirect) o el archivo es fuente .ino,
  // el magic byte falla y el update dice "Verify Bin Header Failed".
  safeLog(">>> [OTA] Validando binario remoto...\n");
  WiFiClientSecure checkClient;
  checkClient.setInsecure();
  HTTPClient httpCheck;
  httpCheck.setFollowRedirects(HTTPC_FORCE_FOLLOW_REDIRECTS);
  httpCheck.begin(checkClient, OTA_FIRMWARE_URL);
  int checkCode = httpCheck.GET();
  int binSize = httpCheck.getSize();
  safeLog(">>> [OTA] Bin HTTP=%d size=%d bytes\n", checkCode, binSize);
  if (checkCode == 200 && binSize > 0) {
    WiFiClient* stream = httpCheck.getStreamPtr();
    uint8_t magic = 0;
    unsigned long tWait = millis();
    while (!stream->available() && (millis() - tWait) < 3000) delay(10);
    if (stream->available()) {
      magic = (uint8_t)stream->read();
      safeLog(">>> [OTA] Magic byte=0x%02X (%s)\n", magic,
              magic == 0xE9 ? "OK - binario valido" : "INVALIDO - subir .bin compilado correcto");
      if (magic != 0xE9) {
        httpCheck.end();
        safeLog(">>> [OTA] Abortando: el archivo en GitHub NO es un .bin de ESP32\n");
        safeLog(">>> [OTA] En Arduino IDE: Sketch -> Export Compiled Binary, subir el .ino.bin renombrado a sonda_14.bin\n");
        return;
      }
    } else {
      safeLog(">>> [OTA] No se pudo leer stream del binario\n");
      httpCheck.end();
      return;
    }
  } else {
    safeLog(">>> [OTA] No se pudo descargar binario (HTTP %d)\n", checkCode);
    httpCheck.end();
    return;
  }
  httpCheck.end();
